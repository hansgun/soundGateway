...
class Deep_SVDD(nn.Module):
    def __init__(self, z_dim=32):
        ...
        self.fc1 = nn.Linear(150660, z_dim, bias=False)
...
def data_loader(train_dir, test_dir, batch_size, trans, train_labels=0, test_labels=0) :
...
#     train_dataset, validation_dataset, test_dataset = random_split(train_data, [train_size, validation_size, test_size])
    train_data = Img_Dataset(root=train_dir, transform=trans, labels=train_labels)
...
if __name__ == '__main__' : 
    # DNS
    BASE_PATH = '/home/centos/data/sound/ib/1st/datas/jpg_1s/' # spectrogram
    ...
    # load label data
    with open('./specto_normal.pickle','br') as f : 
    ...
    data = data_loader(TRAIN_PATH, TEST_PATH, args.batch_size, trans, train_labels=train_label['divide'].to_list(), test_labels=test_label['divide'].to_list())

    isTrain=True # True for train, False for test
    if isTrain : 
        ### train code ###############################################################################
        ...
        ### load model ###############################################################################
        deep_SVDD = torch.load('torchModel/deep_SVDD_spectrum_v0.1.pth',map_location={'cuda:0':'cuda:2'})
